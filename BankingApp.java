...
interface BankOperations {
 void deposit(int accNo, double amount) throws
AccountNotFoundException;
void withdraw(int accNo, double amount) throws
AccountNotFoundException, InsufficientBalanceException;
 void createDemandDraft(int accNo, String payee, double amount)
...
class Bank implements BankOperations {
 ...
 private Account findAccount(int accNo) throws
AccountNotFoundException {
 ...
 @Override
 public void deposit(int accNo, double amount) throws
AccountNotFoundException {
 Account acc = findAccount(accNo);
 acc.balance += amount;
 System.out.println(" Deposited ₹" + amount + " into " +
acc.holderName + "'s account.");
}
@Override
 public void withdraw(int accNo, double amount) throws
AccountNotFoundException, InsufficientBalanceException {
 Account acc = findAccount(accNo);
 if (acc.balance >= amount) {
 acc.balance -= amount;
 System.out.println(" Withdrawn ₹" + amount + " from " +
acc.holderName + "'s account.");
 } else {
 ...
 public void createDemandDraft(int accNo, String payee, double amount) throws AccountNotFoundException, InsufficientBalanceException
{
 ...
 if (acc.balance >= amount) {
 acc.balance -= amount;
 System.out.println(" Demand Draft created for ₹" + amount +
" in favor of " + payee + " from " + acc.holderName + "'s account.");
 } else {
